...
export default function ConversationCenter() {
  ...
  const isAdmin = (auth.claims?.role ?? "user").toLowerCase() === "admin";
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      const response = await authorizedFetch(`${API_URL}/conversation/contacts`);
      if (response.ok) {
        const data = (await response.json()) as Contact[];
        setContacts(data);
      }
    } catch (error) {
      console.error("[Conversation] Failed to load contacts", error);
    }
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      const endpoint = isAdmin ? `${API_URL}/conversation?includeAllForStaff=true` : `${API_URL}/conversation`;
      ...
      if (response.ok) {
        ...
        console.error("[Conversation] Failed to fetch conversations", response.status);
      }
    } catch (error) {
  ...
  }, [authorizedFetch, auth.tokens?.accessToken, isAdmin]);
  ...
  const markConversationRead = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        console.error("[Conversation] Failed to mark read", error);
      }
    },
    [authorizedFetch, auth.tokens?.accessToken],
  );
  ...
  const startConnection = useCallback(async () => {
    ...
    try {
      ...
      connection.onreconnected(async () => {
        ...
        if (conversationId) {
          try {
            ...
            console.error("[Conversation] Failed to rejoin conversation", error);
          }
        }
      });
      ...
      console.error("[Conversation] Failed to connect", error);
      setConnectionState("error");
      setConnectionError(error instanceof Error ? error.message : "Connection failed");
    }
  }, [auth.tokens?.accessToken, currentUserId, fetchConversations, markConversationRead]);
    ...
    async (conversationId: string) => {
      ...
      if (connectionRef.current && connectionRef.current.state === signalR.HubConnectionState.Connected) {
        try {
          ...
          console.error("[Conversation] Failed to join conversation", error);
        }
      }
  ...
  const handleCreateConversation = useCallback(async () => {
    ...
    try {
      const response = await authorizedFetch(`${API_URL}/conversation`, {
        ...
        body: JSON.stringify({ participantIds: [selectedContactId] }),
      });

      if (!response.ok) {
        const error = await response.json().catch(() => ({ message: "Failed to start conversation" }));
        throw new Error((error as { message?: string }).message ?? "Failed to start conversation");
      }
  ...
  }, [authorizedFetch, fetchMessages, selectedContactId]);
  ...
  }, [auth.tokens?.accessToken, fetchConversations, fetchContacts, startConnection]);
            ...
            >
              ...
              {contacts.map((contact) => (
                <option key={contact.id} value={contact.id}>
                  {contact.name} {contact.role === "Admin" ? "(Staff)" : ""}
                </option>
              ))}
            </select>
