...
export default function CustomerChat() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setContacts(data.filter((c) => c.role === "Admin"));
      }
    } catch (error) {
      console.error("[Customer Chat] Failed to load contacts", error);
    }
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Customer Chat] Failed to fetch conversations", error);
    } finally {
  ...
  const fetchMessages = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        console.error("[Customer Chat] Failed to fetch messages", error);
      } finally {
  ...
  const markConversationRead = useCallback(
    async (conversationId: string) => {
      ...
      try {
        ...
        console.error("[Customer Chat] Failed to mark read", error);
      }
    },
    [authorizedFetch, auth.tokens?.accessToken],
  );
  ...
  const startConnection = useCallback(async () => {
    ...
    try {
      ...
      connection.onreconnected(async () => {
        ...
        if (conversationId) {
          try {
            ...
            console.error("[Customer Chat] Failed to rejoin conversation", error);
          }
        }
      });
      ...
      console.error("[Customer Chat] Failed to connect", error);
    ...
    async (conversationId: string) => {
      ...
      if (connectionRef.current && connectionRef.current.state === signalR.HubConnectionState.Connected) {
        try {
          ...
          console.error("[Customer Chat] Failed to join conversation", error);
        }
      }
  ...
  const handleCreateConversation = useCallback(async () => {
    ...
    try {
      ...
      console.error("[Customer Chat] Failed to start conversation", error);
      ...
      <Card className="flex h-full flex-col">
        <CardHeader className="space-y-3 pb-3">
          ...
          <div className="flex gap-2">
            ...
            >
              <option value="">Contact support staff</option>
            ...
            <Button onClick={handleCreateConversation} disabled={!selectedContactId || isCreatingConversation}>
              {isCreatingConversation ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
            </Button>
          </div>
        </CardHeader>
