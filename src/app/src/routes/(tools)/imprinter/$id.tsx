...
import { ImprinterProvider } from "#/routes/(tools)/imprinter/-components/hooks/use-imprinter";
import type { AppliedDesign, CameraState, ProductModel } from "#/routes/(tools)/imprinter/-types";

export const Route = createFileRoute("/(tools)/imprinter/$id")({
  component: Page,
});
...
function Page() {
  ...
  const initialProductId = search["product"];
  ...
  }, [imprintId, isNew, initialDesignId, initialProductId, api.imprint, navigate]);
